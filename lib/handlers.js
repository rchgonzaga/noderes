...
handlers._checks = {
  ...
  /**
   * Tokens - post
   * Required data: protocol, url, successCodes, timeoutSeconds
   * Optional data: none
   */
  post: (data, cb) => {
    ...
    const successCode = typeof (data.payload.successCode) === 'object' && data.payload.successCode instanceof Array && data.payload.successCode.length > 0 ?
          ...
          _data.read('users', userPhone, (err, userData) => {
            if (!err && userData) {
              ...
              if (userChecks.length < config.maxChecks) {
                ...
                cb(400, {
                  'Error': 'The user already have the maximum number of checks: ' + config.maxChecks
                })
              }
            } else {
              cb(403)
