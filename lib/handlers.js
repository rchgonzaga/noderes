...
handlers._checks = {
  ...
  /**
   * Tokens - post
   * Required data: protocol, url, sucessCodes, timeoutSeconds
   * Optional data: none
   */
  post: (data, cb) => {
    ...
    const method = typeof (data.payload.method) === 'string' && ['post', 'get', 'put', 'delete'].indexOf(data.payload.protocol) > -1 ?
      data.payload.method :
      false

    const sucessCode = typeof (data.payload.sucessCode) === 'object' && data.payload.sucessCode instanceof Array && data.payload.sucessCode.length > 0 ?
      data.payload.sucessCode :
      false
    ...
    if (protocol && url && method && sucessCode && timeoutSeconds) {
      ...
      // Reade the token from FS
      _data.read('tokens', (err, tokenData) => {
        if (!err && tokenData) {
          const userPhone = tokenData.phoneNumber
          _data.read('users', userPhone, (err, userData) => {
            if (!err && userData) {
              const userChecks = typeof (userData.checks) === 'object' && userData.checks instanceof Array ?
                userData.checks : []

              // Verify if the user has less than the max-chekcks-per-user
              if (userChecks.length < config.maxChecks) {
                // Create a rnd id for the check
                ...
                cb(400, {
                  'Error',
                  'The user already have the maximum number of checks: ' + config.maxChecks
                })
              }
            } else {
              cb(403)
